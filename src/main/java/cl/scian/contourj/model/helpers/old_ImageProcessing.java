...
import cl.scian.contourj.model.VectorField;
...
public final class old_ImageProcessing {

    private old_ImageProcessing() {}

    private static double[][] gradient(int[][] image, int direction) {
        double[][] imageCastToDouble = Arrays.stream(image)
                .map(row -> Arrays.stream(row)
                        .mapToDouble(value -> value)
                        .toArray())
                .toArray(double[][]::new);
        return gradient(imageCastToDouble, direction);
    }

    private static double[][] gradient(double[][] image, int direction) {
    ...
    public static VectorField calculateEdgeMap(int[][] image) {
    ...
    private static double[][] convolution(double[][] image, double[][] kernel) {
    ...
    private static double[][] laplacian(double[][] image) {
    ...
    public static VectorField calculateGGVF(int[][] image, double mu, int gvf_iterations) {

        VectorField edgeMap = calculateEdgeMap(image);
        ...
        for (int iterationNumber = 1; iterationNumber <= gvf_iterations; iterationNumber++) {
            double[][] uLaplacian = old_ImageProcessing.laplacian(u);
            double[][] vLaplacian = old_ImageProcessing.laplacian(v);
